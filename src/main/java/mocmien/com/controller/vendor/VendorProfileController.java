...
public class VendorProfileController {
	...
	public ResponseEntity<?> updateProfile(@AuthenticationPrincipal CustomUserDetails userDetails,
			...
			@RequestParam(required = false) String gender, @RequestParam(required = false) String avatarUrl,
			@RequestParam(required = false) String password) {
		if (userDetails == null) {
			return ResponseEntity.badRequest().body(Map.of("success", false, "message", "Người dùng chưa đăng nhập!"));
		}
		...
		if (currentUser.getUserProfile() != null) {
			if (name != null && !name.isEmpty())
				currentUser.getUserProfile().setFullName(name);
			if (dob != null)
				currentUser.getUserProfile().setDob(dob);
			if (gender != null && !gender.isEmpty())
				currentUser.getUserProfile().setGender(gender);
		}
