...
public class VendorProductsController {
	...
	public Page<ProductListItemResponse> list(@RequestParam(defaultValue = "") String keyword,
		...
		Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "updateAt", "createAt"));
		return queryService.list(keyword, categoryId, status, pageable);
	}
	...
	public ResponseEntity<?> create(@Valid @RequestBody ProductCreateRequest req) {
		...
		p.setSlug(uniqueSlug(slugify(req.getProductName())));
	...
	public ResponseEntity<?> update(@PathVariable Integer id, @Valid @RequestBody ProductUpdateRequest req) {
		...
		p.setCategory(cat);
		p.setProductName(req.getProductName());
		p.setSlug(uniqueSlug(slugify(req.getProductName()), p.getId())); // đảm bảo không trùng
		p.setPrice(req.getPrice());
		p.setPromotionalPrice(req.getPrice());
		p.setSize(req.getSize());
		p.setStock(req.getStock());
	...
	private String uniqueSlug(String base, Integer ignoreId) {
		String s = base;
		int i = 1;
		while (productRepo.findBySlug(s).filter(p -> ignoreId == null || !p.getId().equals(ignoreId)).isPresent()) {
			s = base + "-" + (++i);
		}
		return s;
	}
	...
	private String slugify(String s) {
		if (s == null)
			return null;
		return s.trim().toLowerCase().replaceAll("[^a-z0-9\\s-]", "").replaceAll("\\s+", "-").replaceAll("-{2,}", "-");
	}
