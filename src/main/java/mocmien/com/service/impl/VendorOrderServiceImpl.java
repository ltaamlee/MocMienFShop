...
public class VendorOrderServiceImpl implements VendorOrderService {
	...
	private VendorOrderListItemResponse toListItem(Orders o) {
		...
		r.setTotal(o.getAmountFromCustomer() == null ? BigDecimal.ZERO : o.getAmountFromCustomer());
	...
	public VendorOrderDetailResponse detail(Integer vendorUserId, String orderId) {
		...
		if (o.getOrderDetails() != null) {
			var items = o.getOrderDetails().stream().map(od -> {
				...
				BigDecimal amt = (od.getPrice() == null || od.getQuantity() == null) ? BigDecimal.ZERO
						: od.getPrice().multiply(BigDecimal.valueOf(od.getQuantity()));
				it.setAmount(amt);
				return it;
		...
		BigDecimal shippingFee = (o.getDelivery() != null) ? o.getDelivery().getBasePrice() : BigDecimal.ZERO;
